...
func NewLokiExecutor(baseURL string, options pkg.Options) *LokiExecutor {
	client := &http.Client{
...
func (e *LokiExecutor) ExecuteQuery(ctx context.Context, queryType pkg.QueryType) (pkg.QueryResult, error) {
...
func (e *LokiExecutor) GenerateRandomQuery(queryType pkg.QueryType) interface{} {
...
func (e *LokiExecutor) executeLokiQuery(ctx context.Context, queryInfo map[string]string) (pkg.QueryResult, error) {
	// Формируем URL запроса
	apiPath := fmt.Sprintf("/loki/api/v1/%s", queryInfo["path"])
	queryURL, err := url.Parse(e.BaseURL + apiPath)
	if err != nil {
		return pkg.QueryResult{}, fmt.Errorf("ошибка формирования URL: %v", err)
	}
